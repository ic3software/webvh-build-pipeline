...
if [[ ${#ONLY[@]} -gt 0 ]]; then
  valid_names=(did-hosting-server webvh-witness did-hosting-control webvh-watcher did-hosting-daemon did-hosting-daemon-k8s)
  for item in "${ONLY[@]}"; do
    found=0
...
# we build the UI before any of those daemon/control variants.
if want did-hosting-control || want did-hosting-daemon || want did-hosting-daemon-k8s; then
...
if want did-hosting-daemon-k8s; then
...
check_binary did-hosting-daemon-k8s
...
upload() {
  ...
  local latest_dest="s3://${R2_BUCKET}/${name}/latest/${upload_filename}"
  local versioned_dest="s3://${R2_BUCKET}/${name}/${version_tag}/${upload_filename}"
...
want did-hosting-daemon-k8s && upload did-hosting-daemon-k8s did-hosting-daemon-k8s "$daemon_version" did-hosting-daemon
