...
///
/// - `webvh-daemon`  → `DAEMON_*`
/// - `webvh-server`  → `WEBVH_*`
/// - `webvh-control` → `CONTROL_*`
/// - `webvh-witness` → `WITNESS_*`
/// - `webvh-watcher` → `WATCHER_*`
///
/// Matches the runtime env prefixes documented in
/// `docs/bootstrap_startup.md` — operators using `WEBVH_PUBLIC_URL` to
...
/// (running `webvh-server setup --from daemon-build.toml`).
